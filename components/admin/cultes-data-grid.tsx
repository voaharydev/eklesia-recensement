...
function detailHref(
  ...
  return query
    ? `/admin/cultes/${serviceId}?${query}`
    : `/admin/cultes/${serviceId}`;
}

export function CultesDataGrid({
  services,
  hasActiveFilters,
  searchParams,
  labels,
  ...
  if (services.length === 0) {
    return (
      <p className="text-sm text-muted">
        {hasActiveFilters ? labels.emptyFiltered : labels.empty}
      </p>
    );
  }
            ...
            <tbody className="divide-y divide-border bg-surface">
              {services.map((service) => (
                <tr
                  key={service.id}
                  className={
                    service.cancelled_at ? "bg-surface-muted/60" : undefined
                  }
                >
                  ...
                  <td className="px-4 py-3">
                    <Badge variant="warning">
                      {service.statusCounts.pending}
                    </Badge>
                  </td>
                  <td className="px-4 py-3">
                    <Badge variant="success">
                      {service.statusCounts.accepted}
                    </Badge>
                  </td>
                  <td className="px-4 py-3">
                    <Badge variant="error">
                      {service.statusCounts.declined}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <Link
                      href={detailHref(service.id, searchParams)}
