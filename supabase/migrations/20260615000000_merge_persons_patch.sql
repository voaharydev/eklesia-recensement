-- Étend merge_persons pour appliquer un patch éditable sur le profil maître.
...
begin
  ...
  -- Étape B : branches (patch prioritaire, sinon union).
  ...
  -- Étape C : mettre à jour le maître (household_id inchangé).
  ...
  set
    ...
    email = case
      when p_patch is not null and p_patch ? 'email' then
        case
          when p_patch->>'email' is null or trim(p_patch->>'email') = '' then null
          else trim(p_patch->>'email')
        end
      else m.email
    end,
    phone = case
      when p_patch is not null and p_patch ? 'phone' then
        case
          when p_patch->>'phone' is null or trim(p_patch->>'phone') = '' then null
          else trim(p_patch->>'phone')
        end
      else m.phone
    end,
    role = case
  ...
  -- Étape D : supprimer le doublon.
  delete from public.persons
  where id = p_duplicate;
end;
$$;

comment on function public.merge_persons(uuid, uuid, jsonb) is
  'Fusionne p_duplicate dans p_master avec patch optionnel sur le profil maître.';
